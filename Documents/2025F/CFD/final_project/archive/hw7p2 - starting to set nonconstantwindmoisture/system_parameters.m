...
rise_slope = 1;
fall_slope = -1;
x_mountain_start = floor(N.x_p*20/100);
x_mountain_peak = floor(N.x_p*30/100);
x_mountain_end = floor(N.x_p*90/100);
ambienthumidity = 0;
scalefactor     = 10000;                    %Default = 10000. Vertical scale factor in simulation length units to m, i.e., 1 length unit is 10km
...
criticalmass    = .14;                      %Default = 3.2 for phi_wall=1,phi_in=0. Nonrealistic parameter controlling rain threshold
rainremovalrate = 1;                       %default = 0.6 under 0.5 is 100% removal,  up to 1 is 50%. it skips over cells via round(rainremovalrate*rand)
...
if rainshadow ==1
    x = linspace(-6, 6, N.y_p);      % Adjust range for sharpness
    y = 1 ./ (1 + exp(-x));      % Sigmoid function
    phi_in = ones(N.y_p,1).*y;
end