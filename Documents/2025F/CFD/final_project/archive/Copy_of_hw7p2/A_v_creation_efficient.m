...
for i = 1:Ny_v*Nx_v
    ...
    if v_type(i) == 2
        ...
        elseif BC.v_rig_type == "Neumann"
            ...
            A_v(i,i)      =  Fe_v(i)-Fw_v(i)+Fn_v(i)-Fs_v(i)+2*De+Dw+Dn+Ds;            
            A_v(i,i-Ny_v) = -Fw_v(i)-Dw;             
            % A_v(i,i+Ny_v) =  Fe_v(i)-De;
            A_v(i,i+1)    =  Fn_v(i)-Dn;  
            A_v(i,i-1)    = -Fs_v(i)-Ds; 
            b_v(i)        = -(p_n(i)-p_s(i))/dy + 2*speeds.v_rig*De;   
        end
    ...
    elseif v_type(i) == 4
        ...
        elseif BC.v_bot_type == "Neumann"
            ...
            A_v(i,i)      =  Fe_v(i)-Fw_v(i)+Fn_v(i)-Fs_v(i)+De+Dw+Dn+2*Ds;            
            A_v(i,i-Ny_v) = -Fw_v(i)-Dw;             
            A_v(i,i+Ny_v) =  Fe_v(i)-De;
            A_v(i,i+1)    =  Fn_v(i)-Dn;  
            % A_v(i,i-1)    = -Fs_v(i)-Ds; 
            b_v(i)        = -(p_n(i)-p_s(i))/dy + 2*speeds.v_bot*Ds;
        end
    ...
    elseif v_type(i) == 6
        ...
        elseif BC.v_lef_type == "Neumann"
            ...
            A_v(i,i)      =  Fe_v(i)-Fw_v(i)+Fn_v(i)-Fs_v(i)+De+2*Dw+Dn+Ds;            
            % A_v(i,i-Ny_v) = -Fw_v(i)-Dw;             
            A_v(i,i+Ny_v) =  Fe_v(i)-De;
            A_v(i,i+1)    =  Fn_v(i)-Dn;  
            A_v(i,i-1)    = -Fs_v(i)-Ds; 
            b_v(i)        = -(p_n(i)-p_s(i))/dy + 2*speeds.v_lef*Dw;
        end
    ...
    elseif v_type(i) == 8
        ...
        elseif BC.v_top_type == "Neumann"
            ...
            A_v(i,i)      =  Fe_v(i)-Fw_v(i)+Fn_v(i)-Fs_v(i)+De+Dw+2*Dn+Ds;
                        A_v(i,i-Ny_v) = -Fw_v(i)-Dw;             
            A_v(i,i+Ny_v) =  Fe_v(i)-De;
            % A_v(i,i+1)    =  Fn_v(i)-Dn;  
            A_v(i,i-1)    = -Fs_v(i)-Ds; 
            b_v(i)        = -(p_n(i)-p_s(i))/dy + 2*speeds.v_top*Dn;
        end
    
    %% If the cell has an East face on a No-slip boundary
    elseif v_type(i) == 10
        A_v(i,i)      =   -Fw_v(i)+Fn_v(i)-Fs_v(i)+2*De+Dw+Dn+Ds + 1/dt;
        A_v(i,i+1)    =  Fn_v(i)-Dn;  
        A_v(i,i-1)    = -Fs_v(i)-Ds; 
        A_v(i,i-Ny_v) = -Fw_v(i)-Dw; 
