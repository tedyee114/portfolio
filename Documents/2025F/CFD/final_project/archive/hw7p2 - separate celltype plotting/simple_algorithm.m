...
while II <= II_max && abs(u_change(end)) > u_tol
    ...
    [A_u, b_u] = A_u_creation_efficient(u_guess, v_guess, p_guess, dx, dy, dt, Re, u_type, u_prevTime,  A_u, grids, speeds, BC);
    % [A_u, b_u] = A_u_creation_blocked(u_guess, v_guess, p_guess, dx, dy, dt,...
    % Re, u_type, speeds.u_bot, u_prevTime, u_lef,A_u);
    
    %%Calculate the u star value
    u_star = reshape(A_u\b_u,N.y_u,N.x_u);

    %%Update the u_star value with relaxation
    u_star = u_guess + alpha_u*(u_star-u_guess);

    %% Calculation of v-star %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    [A_v, b_v] = A_v_creation_efficient(u_guess, v_guess, p_guess, dx, dy, dt, Re, v_type, v_prevTime, A_v, grids, speeds, BC); 
    % [A_v, b_v] = A_v_creation_blocked(u_guess, v_guess, p_guess, dx, dy, dt,...
    %         Re, v_type, speeds.v_bot, speeds.v_top, v_prevTime,speeds.v_lef,A_v);

    %%Calculate the v-star value
    v_star = reshape(A_v\b_v,N.y_v,N.x_v);
    ...
    p_correction = reshape(A_p\b_p,N.y_p,N.x_p);
    ...
    if mod(II,20) == 0
        ...
        imagesc(ax4, u_guess); colorbar; caxis([-1 1]*max(abs(u_guess(:))));
        title('Horizontal (u) Velocity')
        set(gca,'YDir','normal')
        
        ax5 = axes('Parent', panel_2, 'Position', [0.05, 0.5, 0.9, 0.3]);
        imagesc(ax5, v_guess); colorbar; caxis([-1 1]*max(abs(v_guess(:))));
        title('Vertical (v) Velocity')
        set(gca,'YDir','normal')
        
        ax6 = axes('Parent', panel_2, 'Position', [0.05, 0.1, 0.9, 0.3]);
        imagesc(ax6, p_guess); colorbar; caxis([-1 1]*max(abs(p_guess(:))));
        title('Pressure')
        set(gca,'YDir','normal')
        colormap jet
        drawnow
