...
function rainshadow_callback(N, base)
    [rainshadow, blocked, BC, phi_in] = begin_rainshadow(N, base);
...
function [rainshadow,blocked, BC, phi_in] = begin_rainshadow(N, base)
    rainshadow = 1;
    blocked = 0;
            BC.u_lef_type = "Dirichlet";            % angled incoming wind
            BC.u_lef_value = 10* ones(N.y_v, 1);
            BC.u_rig_type = "FreeSlip";    % outlet
            BC.u_rig_value = zeros(N.y_v,1);
            BC.u_bot_type = "Dirichlet";            % no slip at ground
            BC.u_bot_value = zeros(N.y_v,1);
            BC.u_top_type = "FreeSlip";    % edge of atmosphere
            BC.u_top_value = zeros(N.y_v,1);

            BC.v_lef_type = "Dirichlet";            % horizontal wind
            BC.v_lef_value = 0;
            BC.v_rig_type = "FreeSlip";    % outlet
            BC.v_rig_value = zeros(N.x_u,1);
            BC.v_bot_type = "Dirichlet";            % no slip at ground
            BC.v_bot_value = zeros(N.x_u,1);  
            BC.v_top_type = "FreeSlip";    % edge of atmosphere
            BC.v_top_value = zeros(N.x_u,1);
    x = linspace(-10, 10, N.y_p);      % Adjust range for sharpness
    phi_in = 1- (1 ./ (1 + exp(-10-x)));      % Sigmoid function, adjusted to have a lot at the bottom
