...
panel_1 = uipanel(fig);
panel_1.Layout.Row = [1, 18];
panel_1.Layout.Column = 3;

panel_2 = uipanel(fig);
panel_2.Layout.Row = [1, 18];
...
submit_btn = uibutton(fig, 'Text', 'Submit', 'Position', [10, 220, 120, 30], 'ButtonPushedFcn', @(btn, event) update_plot(fig, grids, N));
...
function BC = update_plot(fig, grids, N)
    % Retrieve the stored value from guidata
    data = guidata(fig);
    disp(fieldnames(data));  % This will help in debugging if the fields exist
    
    % Function to evaluate input (either a scalar or function with a variable)
    function value = evaluate_input(input_str, grids, N)
        try
            % Replace occurrences of 'x' and 'y' with grids.x_p and grids.y_p respectively
            input_str = strrep(input_str, 'x', 'grids.x_p');  % Replace 'x' with grids.x_p
            input_str = strrep(input_str, 'y', 'grids.y_p');  % Replace 'y' with grids.y_p
            
            % Convert the string expression into a function handle
            func = str2func(['@(grids) ' input_str]);  % Convert string to anonymous function
            
            % Evaluate the function over the grid
            value = func(grids);  % Apply the function over the grid
    
            % If the result is a scalar, expand it to match the grid size
            if isscalar(value)
                if isvector(value)
                    value = value * ones(N.y_p, 1); % For u or v boundary conditions along a column
                else
                    value = value * ones(N.x_p, 1); % For u or v boundary conditions along a row
                end
            end
        catch ME
            % If an error occurs (e.g., invalid expression), display a detailed error message
            error('Error evaluating input "%s": %s', input_str, ME.message);
        end
    end
    
    % Retrieve and evaluate inputs for boundary conditions
    % Ensure you're referencing the handles properly from the stored structure
