...
%% Set the u-cell type
% Cells ON the top boundary
u_type(end,:) = 8; % Points ON the top boundary

% Cells ON the bottom boundary
f_x = find(grids.x_u > x_obs_front); % Points after the obstruction
u_type(1,f_x) = 4; 
f_x = find(grids.x_u <= x_obs_front);
f_y = find(abs(grids.y_u-y_obs_top) < dy/2);
u_type(f_y,f_x+1) = 4; % Points ON the top of the obstruction

% Cells with the West face ON the inlet
f_y = find(grids.y_u > y_obs_top); % Above the obstruction and below the top
u_type(f_y,1) = 6; 
...
v_type(f_y+1,1) = 4; % Points ON the top of the obstruction
